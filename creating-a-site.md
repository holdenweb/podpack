...
It makes `dev.env` from the example on first run, checks it can reach your
**local PostgreSQL** — printing the `createuser`/`createdb` lines if not —
applies the migration history and starts the development server. Not
SQLite: a revision autogenerated against SQLite can be one PostgreSQL
rejects, and authoring against the engine you deploy on is why revisions
are authored here at all.
