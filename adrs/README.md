...
| [ADR-0039](0039-the-hop-count-follows-the-deployment.md) | **The hop count follows the deployment.** Amends ADR-0036. `PODPACK_PROXY_HOPS` was the one variable podpack cannot deliver — its documentation lives in a *seeded* `env.example` that `substrate upgrade` never rewrites — so a real deployment that forgot it served every absolute URL as `http://` and said nothing. The count is derived from `PODPACK_ENVIRONMENT` instead (0 local, 1 otherwise, unrecognised counting as not-local like `prepare-host-dirs.sh`), with the variable surviving as an override for two proxies or for a non-local site behind none, which must now say `0` explicitly. |
| [ADR-0037](0037-a-deployment-is-named-separately-from-its-site.md) | **A deployment is named separately from its site.** *Proposed.* Eleven names for four things, and two of them keep a per-deployment value where it cannot vary: `SITE_NAME` is the compose project, not the site's, and `base_url` sits in committed `app.toml` while production and staging answer on different addresses. `COMPOSE_PROJECT_NAME` is adopted because compose *validates* it — `name: ${SITE_NAME}` accepts `holdenweb.com` and silently yields `holdenwebcom`, which is the root of every naming confusion here. `.env.example` retires: a seeded file cannot deliver a new variable, which is why `PODPACK_PROXY_HOPS` never reached an existing site. |
